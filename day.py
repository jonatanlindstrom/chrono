...
class Day(object):
    def __init__(self, date_string):
        ...
        if self.date.isoweekday() < 6:
            self.expected_hours = datetime.timedelta(hours=8)
        else:
            self.expected_hours = datetime.timedelta()
    ...
    def report_lunch_duration(self, lunch_duration):
        if self.start_time is None:
            raise errors.ReportError("Date {} must have a start time before a "
                "lunch duration can be reported.".format(
                    self.date.isoformat()))
    ...
    def report_end_time(self, end_time):
        if self.start_time is None:
            raise errors.ReportError("Date {} must have a start time before "
                "an end time can be reported.".format(self.date.isoformat()))

        if self.lunch_duration is None:
            raise errors.ReportError("Date {} must have a lunch duration "
                "before an end time can be reported.".format(
                    self.date.isoformat()))
